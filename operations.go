...
// validitySeconds controls the NotAfter of the signed certificate. Zero means
// use the KMS default. Passed as CertificateValidityEnd (KMIP 2.1 §7.30).
func (c *Client) Certify(ctx context.Context, csrPEM []byte, caKeyUID, caCertUID string, x509Extension []byte, validitySeconds int32) (*CertifyResponse, error) {
	attrs := []TTLV{
		Structure("Link",
	...
	if validitySeconds > 0 {
		notAfter := time.Now().Add(time.Duration(validitySeconds) * time.Second)
		attrs = append(attrs, Structure("CertificateAttributes",
			DateTime("CertificateValidityEnd", notAfter.UTC().Format(time.RFC3339)),
		))
	}
